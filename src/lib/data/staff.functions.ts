 ...
 */
export const getStaffDashboard = createServerFn({ method: "GET" }).handler(
  async () => {
    ...
    const [
      ...
      db
        .from("donations")
        .select(
          "id, member_discord_id, staff_username, total_final, status, created_at",
        )
        .order("created_at", { ascending: false })
        .limit(8),
      db
    ...
    if (topIds.length > 0) {
      ...
      const byId = new Map(
        (members ?? []).map((m: any) => [m.discord_id, m]),
      );
