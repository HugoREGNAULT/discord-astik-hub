  ...
  return (
    <div className="max-w-6xl space-y-5">
      <ToolHeader
        ...
        description="Historique des serveurs Paladium (snapshot toutes les 15 min). Disponibilité et joueurs en ligne sur 7 jours."
      />

      {q.isLoading && <LoadingBlock />}
      {q.error && <ErrorBlock message={(q.error as Error).message} />}
      {!q.isLoading && grouped.length === 0 && (
        <EmptyBlock label="Aucun snapshot — attends le prochain passage du cron (15 min)." />
      )}
          ...
          {current && (
            <ToolCard>
              <div
                ...
                style={{ fontFamily: "'Space Mono'" }}
              >
                // {current.label} — joueurs en ligne (7j)
              </div>
              <div className="h-64">
