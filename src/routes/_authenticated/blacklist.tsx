...
function BlacklistPage() {
    ...
    return (
      <div className="min-w-0">
        ...
        <div className="flex items-center gap-1.5 group">
          <span className={`text-sm break-all ${mono ? "font-mono" : ""}`} title={value}>
            {value}
          </span>
          <button
            type="button"
  ...
  function BlacklistEntryRow({ entry }: { entry: BlacklistRow }) {
    ...
    const avatarUrl = headId
      ? `https://mc-heads.net/avatar/${encodeURIComponent(headId)}/64`
      : null;
    const nameLookup = entry.mc_name
      ? `https://fr.namemc.com/search?q=${encodeURIComponent(entry.mc_name)}`
      : null;
      ...
      <li className="p-4 flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0 space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            Ajouté par{" "}
            <span className="font-medium text-foreground/80">{entry.added_by_username ?? "?"}</span>
            {" · "}
      ...
      >
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Au moins un identifiant doit rester renseigné.</DialogDescription>
          </DialogHeader>
